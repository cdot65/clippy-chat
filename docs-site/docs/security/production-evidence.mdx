...
[Forgejo issue #22](https://git.cdot.io/cdot.io/clippy-chat/issues/22), which tracked moving that
credential *into* **AI Security Academy - Runtime**, was closed as invalid — it required exactly the
coupling this boundary forbids. The evidence above predates that change and is unaffected by it:
no issuer, audience, client, scope, or workspace value moved.
