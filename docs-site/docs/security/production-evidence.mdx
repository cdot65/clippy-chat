...
## Open secret-placement gap

These results prove request authorization, not final secret-store ownership. The live
`clippy-mcp-client` Secret currently reconciles from a Truffles-vault item. The authoritative
design requires **AI Security Academy - Runtime** only. This is tracked in
[Forgejo issue #22](https://git.cdot.io/cdot.io/clippy-chat/issues/22) and blocks final
secret-boundary approval until migration plus a repeated E2E matrix.
