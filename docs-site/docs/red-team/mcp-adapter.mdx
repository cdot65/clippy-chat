...
The full adapter lives at [`redteam/clippy_redteam_mcp.py`](https://github.com/cdot65/clippy-chat/blob/main/redteam/clippy_redteam_mcp.py).
...
POST http://clippy-mcp.clippy.svc.cluster.local:8080/mcp
  streamable HTTP, stateless, NO auth (cluster-internal only)
...
All configuration is through adapter **vars** — there are no secrets (the MCP server is
unauthenticated).
