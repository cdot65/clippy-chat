...
| `M2M_AUDIENCE` | string — the `aud` a bearer token **must** carry | optional | unset — audience unchecked until a Keycloak audience mapper exists (see below) |
...
# Expected aud on machine bearer tokens. Leave unset until the Keycloak client's
# dedicated scope mints this audience — arming it early 401s every machine caller.
...
- **`M2M_AUDIENCE`** adds a second, independent check: when set, `verifyBearer` requires `aud` and
  ...
  the same realm no longer authenticates. It is unset in production on purpose — Keycloak mints no
  useful `aud` on a `client_credentials` token until the calling client's dedicated scope carries an
  audience mapper, so arming it first would `401` every machine caller. Mapper first, env var
  second: `k8s/m2m-audience-rollout.md`.
